(function () {
	...
	Ext.define('Ext.ux.ColumnAutoWidthPlugin', {
		...
		resume: function (refresh) {
			...
			if (refresh) this.viewChangeDT.delay(0);
		},
		...
		refresh: function () {
			...
			if (grid.view.isExpandingOrCollapsing) {
				me.viewChangeDT.delay(0);
				return;
			}
			var cols = me.getAutoCols();
			console.log('autocolumnwidth refresh', cols);
			if (cols.length) me.doAutoSize(cols);
		},
		getAutoCols: function () {
			var me = this;
			var cols = (me.grid.normalGrid ? me.grid.normalGrid.headerCt : me.grid.headerCt).gridDataColumns;
			var out = [];
			var i = cols.length;
			var col;
		...
		doAutoSize: function (resizeCols) {
			...
			Ext.batchLayouts(function () {
				...
				Ext.each(resizeCols, function (col) {
					...
					col.newWidth = newWidth;
					totalWidth += newWidth;
				});
				
				var factor = 1;
				var gridWidth = me.grid.getView().getEl().dom.clientWidth;
				if (totalWidth < gridWidth) {
					factor = gridWidth / totalWidth;
				}

				Ext.each(resizeCols, function (col) {
					var newWidth = Math.floor(col.newWidth * factor) - 2;
