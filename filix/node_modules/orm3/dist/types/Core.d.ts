 ...
 */
export type HookFunctionWithOptions<T = unknown> = (options: T) => Promise<void>;
/**
 ...
 */
export type Validator<T = unknown> = (value: T) => Promise<void | Error>;
/**
 ...
 */
export type ConnectCallback<T = unknown> = (err: Error | null, db?: T) => void;
/**
 ...
 */
export type PropertyType = 'text' | 'number' | 'integer' | 'float' | 'boolean' | 'date' | 'binary' | 'object' | 'enum' | 'point' | 'serial';
/**
...
export interface ChainFind<T = any> extends PromiseLike<Instance<T>[]> {
    ...
    each(cb: (instance: Instance<T>, index: number, items: Instance<T>[]) => void | Promise<void>): ChainInstanceCalls<Instance<T>>;
    model: Model<T>;
    options: Record<string, unknown>;
    then<TResult1 = Instance<T>[], TResult2 = never>(onfulfilled?: (value: Instance<T>[]) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>): Promise<TResult1 | TResult2>;
    catch<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>): Promise<Instance<T>[] | TResult>;
}
/**
