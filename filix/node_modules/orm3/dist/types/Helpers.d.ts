 ...
 */
export declare function promiseToCallback<T>(promise: Promise<T>, callback: (err: Error | null, result?: T) => void): void;
/**
 * Convert callback to promise
 */
export declare function callbackToPromise<T>(fn: (cb: (err: Error | null, result?: T) => void) => void): Promise<T>;
/**
 ...
 * Make function async
 */
export type AsyncFunction<T extends (...args: any[]) => any> = (...args: ParametersOf<T>) => Promise<ReturnTypeOf<T>>;
/**
 * Array element type
 */
 ...
 */
export type AsyncOf<T> = T extends Promise<infer U> ? Promise<U> : Promise<Awaited<T>>;
/**
 ...
 */
export type IfUnknown<T, A, B = T> = unknown extends T ? T extends unknown ? A : B : B;
/**
 ...
 */
export type ToSnakeCase<S extends string> = S extends `${infer F}${infer R}` ? F extends Uppercase<F> ? `_${Lowercase<F>}${ToSnakeCase<R>}` : `${F}${ToSnakeCase<R>}` : S;
