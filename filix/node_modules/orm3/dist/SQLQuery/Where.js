...
function build(dialect, where, opts) {
    if (!where.length) {
        return [];
    }
    const clauses = [];
...
function buildOrGroup(dialect, where, opts) {
    ...
    for (const key in where.w) {
        ...
        if (typeof value.sql_comparator === 'function') {
            ...
            switch (comparator) {
                case 'between':
                    query.push(`${buildComparisonKey(dialect, where.t, key)} BETWEEN ${dialect.escapeVal(value.from, options.timezone)} AND ${dialect.escapeVal(value.to, options.timezone)}`);
                    break;
                case 'not_between':
                    query.push(`${buildComparisonKey(dialect, where.t, key)} NOT BETWEEN ${dialect.escapeVal(value.from, options.timezone)} AND ${dialect.escapeVal(value.to, options.timezone)}`);
                    break;
                case 'like':
                    query.push(`${buildComparisonKey(dialect, where.t, key)} LIKE ${dialect.escapeVal(value.expr, options.timezone)}`);
                    break;
                case 'not_like':
                    query.push(`${buildComparisonKey(dialect, where.t, key)} NOT LIKE ${dialect.escapeVal(value.expr, options.timezone)}`);
                    break;
                case 'eq':
                case 'ne':
                case 'gt':
                ...
                case 'not_in': {
                    ...
                    query.push(`${buildComparisonKey(dialect, where.t, key)} ${op} ${dialect.escapeVal(value.val, options.timezone)}`);
                    break;
                }
                case 'sql':
                    if (typeof value.where === 'object' && value.where) {
                        let sql = value.where.str.replace('?:column', buildComparisonKey(dialect, where.t, key));
                        sql = sql.replace(/\?:(id|value)/g, (m) => {
...
function normalizeSqlConditions(dialect, queryArray) {
    ...
    return (0, Helpers_1.escapeQuery)(dialect, queryArray[0], queryArray[1]);
}
