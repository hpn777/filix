...
exports.sqlToTessioExample = exports.sqlToTessio = exports.SqlToTessioConverter = void 0;
class SqlToTessioConverter {
    warnings = [];
    unsupportedFeatures = [];
    subQueryCounter = 0;
    /**
     ...
     */
    isCorrelatedSubquery(subquerySql, outerSql) {
     ...
     */
    buildSessionConfig(parsed, subQueries) {
        const config = {
            table: parsed.from
        };
     ...
     */
    parseSelectClause(selectClause, subQueries) {
     ...
     */
    parseWhereClause(whereClause, subQueries) {
        const filters = [];
     ...
     */
    parseCondition(condition, subQueries) {
     ...
     */
    addAggregationColumns(config, selectClause) {
        ...
        const hasGroupByField = config.columns.some(col => config.groupBy?.some(group => group.dataIndex === col.name));
     ...
     */
    handleSubqueries(config, subQueries) {
     ...
     */
    splitByLogicalOperator(clause, operator) {
        const parts = [];
        let current = '';
        let parenLevel = 0;
        let inQuotes = false;
...
exports.SqlToTessioConverter = SqlToTessioConverter;
/**
 * Utility function to convert SQL to Tessio session config
 */
function sqlToTessio(sql) {
    const converter = new SqlToTessioConverter();
    return converter.convertSql(sql);
}
exports.sqlToTessio = sqlToTessio;
/**
 * Utility function to generate conversion examples
 */
function sqlToTessioExample(sql) {
    const converter = new SqlToTessioConverter();
    return converter.generateExample(sql);
}
exports.sqlToTessioExample = sqlToTessioExample;
//# sourceMappingURL=sqlToTessioConverter.js.map