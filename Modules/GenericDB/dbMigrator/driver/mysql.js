...
var MysqlDriver = Base.extend({
	...
	removeTable: function (tableName, callback) {
	...
	addForeignKey: function (tableName, referencedTableName, columnName, referencedColumnName, rules, callback) {
		if (arguments.length === 5 && typeof (rules) === 'function') {
			callback = rules;
			rules = {};
		}
		var constraintName = tableName + '_' + columnName + '_fk';
		var sql = util.format('ALTER TABLE `%s` ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s) ON DELETE %s ON UPDATE %s',
		  tableName, constraintName, columnName, referencedTableName, referencedColumnName, rules.onDelete || 'NO ACTION', rules.onUpdate || 'NO ACTION');
		this.runSql(sql, callback);
	},

	removeForeignKey: function (tableName, constraintName, callback) {
		var sql = util.format('ALTER TABLE `%s` DROP FOREIGN KEY `%s`', tableName, constraintName);
		this.runSql(sql, function () {
			sql = util.format('ALTER TABLE `%s` DROP INDEX `%s`', tableName, constraintName);
			this.runSql(sql, function () {
				callback();
			});
		}.bind(this));
